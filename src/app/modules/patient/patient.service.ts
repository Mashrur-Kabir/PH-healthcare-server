...
import { parseDateString } from "../../helpers/dateTimeConvert";
...
const updateMyProfileInDB = async (
  ...
  if (!payload) {
    throw new AppError(status.BAD_REQUEST, "Payload is missing!");
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (payload.patientHealthData) {
      ...
      if (payload.patientHealthData.dateOfBirth) {
        healthDataToSave.dateOfBirth = parseDateString(
