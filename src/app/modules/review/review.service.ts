...
import { IAuthUser, IQueryParams } from "../../interfaces";
...
const getAllReviewsFromDB = async (query: IQueryParams) => {
  ...
  const result = await queryBuilder
    .search()
    .filter()
    .dynamicInclude(reviewIncludeConfig, ["doctor", "patient", "appointment"]) // Default includes
    .paginate()
    .sort()
    .fields()
    .execute();
...
const myReviewsFromDB = async (user: IAuthUser) => {
...
const deleteReviewFromDB = async (user: IAuthUser, reviewId: string) => {
  const patientData = await prisma.patient.findUniqueOrThrow({
    where: {
      email: user?.email,
    },
