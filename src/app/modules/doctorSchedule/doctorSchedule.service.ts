...
const createMyDoctorScheduleInDB = async (
  user: IAuthUser,
...
const getMyDoctorSchedulesFromDB = async (
  user: IAuthUser,
  query: IQueryParams,
) => {
  const doctorData = await prisma.doctor.findUniqueOrThrow({
...
const getAllDoctorSchedulesFromDB = async (query: IQueryParams) => {
  const queryBuilder = new QueryBuilder<
    DoctorSchedules,
    Prisma.DoctorSchedulesWhereInput,
    Prisma.DoctorSchedulesInclude
...
const getDoctorScheduleByIdFromDB = async (
  doctorId: string,
  scheduleId: string,
) => {
...
const updateMyDoctorScheduleInDB = async (
  user: IAuthUser,
...
const deleteMyDoctorScheduleFromDB = async (id: string, user: IAuthUser) => {
  const doctorData = await prisma.doctor.findUniqueOrThrow({
    where: {
      email: user.email,
    },
